...
for (year in 2000:2016){
  ...
  use_stack <- stack(z_tif, new_tif)
  names(use_stack) <- c("Z Dir", "New")
  ...
  diffplot <- levelplot(new_z_diff,
                        par.settings=rasterTheme(region= brewer.pal(10, "RdBu")), at=seq(-maxval,maxval, 0.05),
                        xlab=NULL, ylab=NULL, scales=list(draw=F), margin=F, layout=c(1,1), main="New Minus Z")
