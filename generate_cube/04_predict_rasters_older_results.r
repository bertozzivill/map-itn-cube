###############################################################################################################
## 04_predict_rasters_older_results.r
...
## This version of the script is designed for the older results framework, before the direct 
## stock-and-flow-to-cube data pipeline.
...
predict_rasters <- function(input_dir, main_indir, cov_dir, main_outdir, func_dir, prediction_years){
  ...
  stock_and_flow <- fread(file.path(main_indir, "01_stock_and_flow_access.csv"))
  setnames(stock_and_flow, "emplogit_nat_access", "emp_nat_access")
  ...
  prediction_outcomes <- foreach(this_year=prediction_years,.combine=rbind) %dopar% {
    ...
    all_predictions <- rbindlist(lapply(names(inla_outputs), function(output_var){ # should be "access_dev", "use_gap"
...
if (Sys.getenv("run_individually")!=""){
  ...
  predict_rasters(input_dir, main_indir, cov_dir, main_outdir, func_dir, prediction_years=2000:2018)
