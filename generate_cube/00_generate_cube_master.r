...
# current dsub:
# dsub --provider google-v2 --project my-test-project-210811 --image gcr.io/my-test-project-210811/map_geospatial --regions europe-west1 --label "type=itn_cube" --machine-type n1-hihgmem-64 --logging gs://map_data_z/users/amelia/logs --input-recursive input_dir=gs://map_data_z/users/amelia/itn_cube/input_data cov_dir=gs://map_data_z/cubes_5km func_dir=gs://map_data_z/users/amelia/itn_cube/code/generate_cube/ --input CODE=gs://map_data_z/users/amelia/itn_cube/code/generate_cube/00_generate_cube_master.r --output-recursive main_dir=gs://map_data_z/users/amelia/itn_cube/results/20190614_rearrange_scripts/ --command 'Rscript ${CODE}'
...
prep_covariates(input_dir, cov_dir, func_dir, main_indir=main_dir, main_outdir=main_dir, prediction_years=start_year:end_year)
toc <- Sys.time()
time_passed(tic, toc)
...
run_dev_gap_models(input_dir, func_dir, main_indir=main_dir, main_outdir=main_dir, start_year, end_year-1) # TODO: move to just end_year eventually
toc <- Sys.time()
time_passed(tic, toc)
...
predict_rasters(input_dir, cov_dir, func_dir, main_indir=main_dir, main_outdir=main_dir, prediction_years=start_year:end_year)
toc <- Sys.time()
time_passed(tic, toc)
