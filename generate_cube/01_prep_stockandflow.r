...
load(file.path(main_dir, "net_probs_and_means.rData")) # contains objects names "out" (list of stock and flow time series) and "Cout" (country names)
...
write.csv(stock_and_flow, file=file.path(output_dir, "01_stock_and_flow_probs_means.csv"), row.names=F)
...
hh_sizes<-fread(file.path(main_dir, "HHsize.csv"))
survey_key=fread(file.path(main_dir, "hh_surveys_key.csv"))
